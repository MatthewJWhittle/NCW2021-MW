...
```{r}
require(automap)
risk_2019 <- risk_2019 %>% st_transform(crs = 3857) %>% as_Spatial()
world_grid <- world_grid %>% st_transform(crs = 3857) %>% as_Spatial()

risk_krige <- automap::autoKrige(formula = days ~ 1,input_data = risk_2019, new_data = world_grid)
...
risk_raster <- krige_to_raster(risk_krige)
plot(risk_raster)
```
...
plots <-
  ...
  ) %>% set_names(names(annual_risk_kriges))


grid.arrange(plots$year_1995, plots$year_2017)
```
